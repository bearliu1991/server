...
class SDK {
  ...
  async getJsApiTicket() {
    ...
    if (typeof data.expireTime === 'undefined' || data.expireTime < time) {
      ...
      const ticket = await fetch.get(url).then(response => response.data)
        .catch((error) => {
          if (error.code) {
            return error;
          }
  ...
  async getAccessToken() {
    ...
    if (typeof data.expireTime === 'undefined' || data.expireTime < time) {
      ...
      const res = await fetch.get(url).then(response => response.data)
        .catch((error) => {
          if (error.code) {
            return error;
          }
...
module.exports = new SDK(wxConfig.appId, wxConfig.secret);